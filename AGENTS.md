...
- **Port forwarding types**: `src/forward.rs` — `ForwardSpec` (`host`, `guest`, `binds: Vec<BindTarget>`) with its `HOST[:GUEST]` string parser and `ssh_forward_args` (`-L` rendering), `BindTarget` (an IP or `*`), active-forwards state file I/O, supervisor `kill_supervisor`/`kill_all_and_clear` helpers
...
- **`agv forward` uses SSH `-L` tunnels with an agv-spawned supervisor.** Each forward is its own long-lived child process running a loop around `ssh -N -L PORT:localhost:PORT` so it survives transient SSH failures (sshd hiccup, brief network blip). The supervisor is detached into its own process group; stopping a forward sends `SIGTERM` to that group, killing both the supervisor and any in-flight ssh. SSH (rather than QEMU hostfwd) is required because user-mode hostfwd cannot reach guest services bound to `127.0.0.1` — SSH resolves `localhost` from inside the guest. Host<->guest string specs use the form `HOST[:GUEST]` (a `/PROTO` suffix is now rejected — TCP is implicit). State tracked in `<instance>/forwards.toml` with origin (`config`/`adhoc`/`auto`) and supervisor `pid` so `--list` and reconcile can distinguish and sweep dead entries.

- **`agv forward` is persistent by default and state-independent.** It edits `config.forwards` in `<instance>/config.toml` *and* applies the change live when the VM is running, so keeping a forward no longer means stopping the VM. It works on a stopped or suspended VM too (config-only; `open_editable` refuses just `creating`/`configuring`, where another writer owns the config). `--temporary` is the opt-out — "this boot only, don't touch config" — and means the same on both verbs, so `add`/`rm` round-trip. It requires a running VM, since on a stopped one it would be a silent no-op. Defaults are picked per verb by which mistake is cheaper: a wrongly-persistent add costs a stray config line, a wrongly-ephemeral one costs a forward that silently vanishes on restart; for removal that inverts, so a bare `--rm` confirms via `interactive::confirm` — but only when the saved config is actually at stake (`--rm --temporary` and a VM with no saved forwards don't ask). `--rm` matches by host port so a port's binds go as a unit, and resolves every named port against both the live set and the config *before* mutating, so one typo can't half-apply. `--stop` remains a hidden clap alias for `--rm`. Adds persist before spawning: the config is the durable record, and a failed spawn is recoverable with `--reapply`, whereas a live tunnel with no config entry would vanish at the next stop.

- **`agv forward --reapply` is additive, never a reset.** It spawns config forwards that aren't currently live and leaves everything else alone — the undo for `--rm --temporary`. It deliberately does *not* reuse `apply_config_forwards` (the start/resume path), which tears everything down first: that would kill ad-hoc forwards as collateral and re-run `apply_auto_forwards`, which allocates a *fresh* host port and rewrites `<instance>/<name>_port` — silently moving an `auto_forwards` port out from under whatever held it, from a command that reads like a harmless refresh. Stop/start is the reset path. Skips are per-entry and non-fatal, matching how start already treats spawn failures.

- **`agv config set` stays stopped-only for every field, forwards included.** Relaxing the gate for forwards alone would make the surface lumpy (the idle knobs are equally "could apply live") — `config set` is the offline declarative setter, `agv forward` is the live-aware one. `ensure_configurable` appends a pointer to `agv forward` when `--forwards` is present and the VM is running; appended rather than substituted, because `--memory 8G --forwards 8080` is blocked by the memory change too and a forwards-only message would send the user in circles.

- **Forwards can bind past loopback.** By default `ssh -L` binds the host side to `127.0.0.1`, so the key-gated tunnel is the only access path. A forward may carry one or more `BindTarget`s (an IP, or `*` for all interfaces/both families) to listen elsewhere — declared per-forward via the `[[forwards]]` table's `bind` field (scalar or list), a string spec's `@BIND` suffixes (`"8642@0.0.0.0"`, repeatable — `"8642@10.0.0.1@::1"`; works in `forwards = [...]` and `agv config set --forwards`), or `agv forward <vm> <port> --bind <addr>` (repeatable). The supervisor emits one `-L bind:host:localhost:guest` per address (IPv6 bracketed) on a single ssh, adds `GatewayPorts=yes` for non-loopback binds, and flips `ExitOnForwardFailure` to `no` only when a forward has **more than one** bind (partial-failure tolerance on a multi-homed host); a single or loopback forward keeps `=yes` so it respawns on a bind error. Duplicate detection (`validate_unique` / `add`) keys on `(bind-address, host-port)`, with the default loopback keyed as `127.0.0.1` so it collides with an explicit `bind = "127.0.0.1"`. `rm` by host port removes every forward on that port (all binds). The non-loopback warning goes to stderr via one shared helper (`forward::warn_non_loopback_binds`), shown even under `--quiet` (`agv forward`, start/resume, and `config set` paths). The string form splits on every `@` (the `@` disambiguates IPv6 colons that would otherwise collide with `HOST:GUEST`), so it now expresses multiple binds under one supervisor just like the table form; empty segments are rejected so a typo can't silently drop a bind. `--bind` only overrides a spec's binds when actually passed, and combining it with an `@BIND` suffix is an error rather than a silent winner-picks. `config.forwards` is structured (`Vec<ForwardSpec>`, custom (de)serialized as an array-of-tables) rather than a normalized string list, so binds survive save/load. Binding past loopback removes agv's auth boundary, so it warns (to stderr, even under `--quiet`) but never blocks — the operator owns the firewalling. `ForwardJson`/`forwards.toml` carry the `binds` for `--list` and `--json`.

- **agv detaches QEMU itself; `-daemonize` is never passed.** QEMU's own
  daemonize forks without exec'ing, and on macOS the Objective-C runtime
  aborts the process if a class is first initialized in the forked child.
  That is reachable from `savevm` — HVF's GIC state save calls into
  Hypervisor.framework, which is Objective-C underneath — so `agv suspend`
  on Apple Silicon killed QEMU with SIGABRT. Upstream has it confirmed and
  unfixed since 2024 (qemu-project/qemu#2515), and it also bites emulated
  targets on macOS, not just suspend. `spawn_detached` uses the same shape
  as the AVF runner and the forward supervisors: own process group, stdio
  to a log file, handle forgotten, PID written from Rust. Two things follow
  from dropping the flag: `-pidfile` goes too (agv writes `pid` itself, so
  it is on disk before `start` returns), and the "QEMU is up" signal that
  `-daemonize`'s parent exit used to give has to be recreated — hence
  `wait_for_qmp_socket`, which also fails fast if the process dies first.
  QEMU's stderr now lands in `<instance>/qemu.log` instead of a pipe nobody
  reads, so a crash says what happened.
...
- **QEMU**: `disk.qcow2`, `pid`, `ssh_port`, `qmp.sock`, `qemu.log` (QEMU's own stdout/stderr — distinct from `serial.log`, which is the guest console; truncated per start), `efi-vars.fd` (aarch64 only).
